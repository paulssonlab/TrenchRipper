...
def make_parquet_index(df,index_columns,index_precisions):
    ...
    parquet_index = sum([df[index_column].astype('uint')*precision_factors[i] for i,index_column in enumerate(index_columns)])
    return parquet_index

def lookup_parquet_index(index_lookup,index_precisions,index_columns):
    converted_index_lookup = np.array([index_lookup[index_column] if index_column in index_lookup.keys() else -1 for index_column in index_columns])
    ...
    index_precisions_upper = index_precisions[:-1][converted_index_lookup_upper[1:]==-1]
    converted_index_lookup_upper[converted_index_lookup_upper==-1] = np.array([sum([9*(10**i) for i in range(precision)]) for precision in index_precisions_upper])

    index_precisions = index_precisions.astype('uint')
    rolling_precisions = np.add.accumulate(index_precisions[::-1])
    precision_factors = (10**rolling_precisions)

    lower_parquet_index = sum([index_column*precision_factors[i] for i,index_column in enumerate(converted_index_lookup_lower[::-1])]).astype('uint')
    upper_parquet_index = sum([index_column*precision_factors[i] for i,index_column in enumerate(converted_index_lookup_upper[::-1])]).astype('uint')
...
def write_delayed_parition(delayed_partition,partition_idx,output_temp_path,engine):
    ...
    computed_partition.to_parquet(output_temp_path + "/temp_output." + str(partition_idx) + ".parquet",engine=engine)
    with open(output_temp_path + "/Lockfile." + str(partition_idx) + ".txt","w") as outfile:
        outfile.write("Done")
    return partition_idx

def to_parquet_checkpoint(dask_controller,dask_df,output_path,engine='pyarrow',overwrite=False):
    ...
    if os.path.exists(output_temp_path):
        ...
        finished_files = [int(path.split(".")[1]) for path in output_temp_path_dirlist if "Lockfile" in path]
    else:
        finished_files = []
    ...
    for i,delayed_partition in enumerate(delayed_partition_list):
        ...
        output_future = dask_controller.daskclient.submit(write_delayed_parition,delayed_partition,partition_idx,output_temp_path,engine)
    ...
    temp_output_file_list = [output_temp_path + "/temp_output." + str(partition_idx) + ".parquet" for partition_idx in partition_list]
    temp_df = dd.read_parquet(temp_output_file_list,engine=engine,calculate_divisions=True)
