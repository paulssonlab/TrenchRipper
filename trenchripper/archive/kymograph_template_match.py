...
class kymograph_cluster:
    ...
    def find_seed_image(self, file_idx):
        """Get an image for each file to act as reference for drift
        measurements. This is to account for the possibility that fiduciary
        markers are not available in the first timepoint or that it is out of
        focus, even though the majority of images may still be fine. Bad
        timepoints are easier to filter at the kymograph stage.
    ...
    def get_drifts(self, file_idx, seed_idx, max_poi_std=5):
        """Find drifts within each hdf5 archive using interest point matching,
        starting from the seed index.
    ...
    def find_seed_image_and_template(self, file_idx):
        ...
        markers are not available in the first timepoint or that it is out of
        focus, even though the majority of images may still be fine. Bad
        timepoints are easier to filter at the kymograph stage. Use points of
        interest to determine a patch of the image to use for template
        matching.
    ...
    def link_drifts(self, file_indices, seed_images, within_file_drifts):
        """Put all drifts of a field of view with respect to first file by
        linking seed drifts for each hdf5 archive.
    ...
    def get_smoothed_y_percentiles(self,file_idx,seed_image_idx,y_percentile,smoothing_kernel_y):
        """For each imported file, computes the percentile along the x-axis of
        the segmentation channel for the previously identified seed image,
        generating a (y,) array. Then performs median filtering of this array
        for smoothing.
    ...
    def triangle_threshold(self,y_percentiles_smoothed,triangle_nbins,triangle_scaling,triangle_max_threshold,triangle_min_threshold):
        """Applies a triangle threshold to a smoothed y percentile array,
    ...
    def get_trench_edges_y(self,y_percentiles_smoothed,triangle_nbins,triangle_scaling,triangle_max_threshold,triangle_min_threshold,y_min_edge_dist):
        """Detects edges in the shape (y,) smoothed percentile array for each
        seed image.
    ...
    def get_trench_ends(self, seed_image_idx, seed_trench_edges_y, y_drifts, orientations,drop_first_row,drop_last_row, padding_y, trench_len_y):
        """Get trench ends (i.e. y-boundaries) for all times in a field of view
        based on seed image segmentation and calculated drift.
    ...
    def get_ends_and_orientations(self,seed_image_idx,drift_future,edges_future,expected_num_rows,top_orientation,orientation_on_fail,y_min_edge_dist,padding_y,trench_len_y):
        """Get orientations and dead ends of trenches that have not drifted out
        of the image at any timepoint.
    ...
    def crop_y_single(self, file_idx, seed_image_idx, orientation_and_initend_future, padding_y, trench_len_y):
        """Crop a single field of view at a single timepoint on the
        segmentation channel for x segmentation.
    ...
    def get_x_midpoints(self,x_percentiles_smoothed,otsu_nbins,otsu_scaling):
        """Given an x percentile array of shape (rows,x), determines the trench
        midpoints of each row array for the seed index.
    ...
    def get_all_in_bounds(self,midpoint_futures,drift_future,trench_width_x,trench_present_thr):
...
#                 kymo_mask = kymo_mask[:,first_idx:last_idx+1]
    ...
    def generate_kymographs(self,dask_controller):
        ...
        for k,fov_idx in enumerate(fov_list):
            ...
            drift_future = dask_controller.futures["FoV Drifts: " + str(fov_idx)]
            seed_image_future = dask_controller.futures["Seed Image Index: " + str(first_file_idx)]
            future = dask_controller.daskclient.submit(self.get_all_in_bounds,midpoint_futures,drift_future,\
